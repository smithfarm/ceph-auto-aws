...
# start.py
#
# Start a Ceph cluster in AWS.
#
# Generate documentation from #PC comments:
#   $ grep -E '^ *#PC' start.py | sed -e 's/#PC //'g   
#
...
#PC * This script (start.py) makes the following assumptions:
...
#PC       really exist
#PC     * "start" operation involves starting instances (nothing else)
...
#PC * Loop over the instances, starting them.
g['volumes'] = []
for i in g['instances']:
    g['ec2_conn'].start_instances( instance_ids=[ i.id ] )
